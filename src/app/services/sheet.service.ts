...
export class SheetService {
  ...
  getSheetByuserId(id:Number,username):Observable<any>{
    ...
    return this.http.get("/api/songsheet/user/"+id,{responseType:'json',params:param,withCredentials:true});
  }
  ...
  createSheet(sheet:sheet):Observable<any>{
    ...
    return this.http.put("/api/user/songsheet",body,{responseType:'json',withCredentials:true,headers:header});
  }

  getSheetBysheetId(id:Number):Observable<any>{
    //通过sheetid获得歌单信息
    return this.http.get("/api/songsheet/info/"+id,{responseType:'json',withCredentials:true});
  }
  ...
  getSheetBysheetIdAndUserid(id:Number,userid:any):Observable<any>{
    ...
    return this.http.get("/api/songsheet/info/"+id,{responseType:'json',withCredentials:true,params:param});
  }

  getMusicBysheetId(id:Number,pagenumber:number,size:number):Observable<any>{
    //通过sheetid获得歌单中的音乐信息
 
    return this.http.get("/api/songsheet/"+id+"?pagenumber="+pagenumber+"&size="+size,{responseType:'json',withCredentials:true});
  }

  collectSheet(sheetid:Number):Observable<any>{
      var param=new HttpParams().set('sheetid',sheetid+'');
      return this.http.put("/api/user/collection/sheet",null,{withCredentials:true,params:param})
  }
  ...
  getSheetBytype(type,pagenumber,size):Observable<any>
  {
    ...
    return this.http.get("/api/sheet/type",{withCredentials:true,params:param})
  }

  getRecommendSheet(pagenumber,size):Observable<any>
  {
    var param=new HttpParams().set('pagenum',pagenumber).set('size',size);
    return this.http.get("/api/recommend/sheet",{withCredentials:true,params:param})
  }

  getUserSheet(userid,username):Observable<any>
  {
    var param=new HttpParams().set('userid',userid).set('username',username);
    return this.http.get("/api/user/creatsheet",{withCredentials:true,params:param})
  }
  ...
  getsheetCount(type):Observable<any>
  {
    ...
    return this.http.get("/api/sheet/type/count",{withCredentials:true,params:param})
  }
  ...
  searchsheet(key,pagenum:number,size:number):Observable<any>
  {
    ...
    return this.http.get("/api/search/sheet",{withCredentials:true,params:param})
  }

  CountSearchsheet(key):Observable<any>
  {
    let param=new HttpParams().set("key",key);
    return this.http.get("/api/search/sheet/pagecount",{withCredentials:true,params:param})
  }

  getAllSheet(pagenum,size):Observable<any>
  {
    let param=new HttpParams().set("pagenum",pagenum).set("size",size);
    return this.http.get("/api/backstage/sheet",{withCredentials:true,params:param})
  }

  getAllSheetCount():Observable<any>
  {
    return this.http.get("/api/backstage/sheet/count",{withCredentials:true})
  }
  ...
  updateAllColumData(s:sheet):Observable<any>
  {
    ...
    var header=new HttpHeaders().set('Content-Type','application/json;charset=UTF-8');
    return this.http.post("/api/backstage/sheet",body,{responseType:'json',withCredentials:true,headers:header});
  }
}
